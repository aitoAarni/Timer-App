...
describe('timers.tsx file', () => {
    ...
    describe('Timer', () => {
        ...
        it('switches to break after active timer runs out', () => {
            timer.pauseToggle()
            addMockTime(20_001)
            timer.updateTimer()
        ...
        it("switches timer's work time for next work time", () => {
            timer.pauseToggle()
            timer.updateTimer()
            timer.setNextWorkTime(35)
            timer.resetTimer()
            timer.pauseToggle()
            timer.resetTimer()
            expect(timer.getSecondsRemaining()).toBe(34)
        })
        it("switches timer's break time for next work time", () => {
            timer.pauseToggle()
            timer.setNextBreakTime(28)
            addMockTime(20_001)
            timer.updateTimer()
            expect(timer.getSecondsRemaining()).toBe(27)
        })
    })
})
